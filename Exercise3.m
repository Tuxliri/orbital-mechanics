...
x = fminunc(@(x) test_optimization(planetA,planetB,x,1,1), [OPT_DEP_MJD2000 OPT_ARR_MJD2000]);
...
[~,R_A_dep,~] = ephemeris(OPT_DEP_MJD2000,planetA);
[~,R_A_arr,~] = ephemeris(OPT_ARR_MJD2000,planetA);
...
[~,R_B_dep,~] = ephemeris(OPT_DEP_MJD2000,planetB);
[~,R_B_arr,~] = ephemeris(OPT_ARR_MJD2000,planetB);
%% Plotting
R = [r_TF_ARC R_A R_B];
...
plot3(R_A_arr(1),R_A_arr(2),R_A_arr(3),'-o','MarkerFaceColor','#D9FFFF','MarkerSize',10)
plot3(R_A_dep(1),R_A_dep(2),R_A_dep(3),'-o','MarkerFaceColor','#D9FFFF','MarkerSize',10)
