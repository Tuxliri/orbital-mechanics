...
MISSION = {'Mercury',1,[2023 11 01; 2025 01 01], [2024 04 01; 2025 03 01], 7.0,[210,210,210];
           ...
           'Neptune -> Venus',2,[2031 02 01; 2046 02 01], [2055 02 01; 2071 02 01], 200, [254,235,186];
           'Venus -> Mercury',1,[2036 02 01; 2036 12 01], [2036 03 01; 2036 12 01],20,[253, 144,148];
           };
...
% Set upper and lower bound constraints for the fmincon function
% these are the earliest and latest departure and arrival dates for the
% mission

ub = [windowPlanetA(2), windowPlanetB(2)];
lb = [windowPlanetA(1), windowPlanetB(1)];

% Departure and arrival dates vector
x0 = [OPT_DEP_0 OPT_ARR_0];

% Constrained optimization solver
x = fmincon(@(x) designer_opt(planetA,planetB,x(1),x(2)),x0,A,b,Aeq,beq,lb,ub, @(x) launcherCONSTR(x(1),x(2),planetA,planetB,vinf) ,options);
...
plot3(datenum(mjd20002date(OPT_DEP_MJD2000)),datenum(mjd20002date(OPT_ARR_MJD2000)),min_DV,'r*');
plot3(datenum(mjd20002date(xGA(1))),datenum(mjd20002date(xGA(2))),min_DVGA,'rx');
plot3(datenum(mjd20002date(x_PRIME(1))),datenum(mjd20002date(x_PRIME(2))),min_DV_PRIME,'ro');
hold off
